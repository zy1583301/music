...
<div class="searchBox">
  ...
  <div class="resultList" v-show="showState" ref="result">
    <ul>
      ...
      <li class="resultItem" v-for="(item,index) in result" :key="index" @click.stop="goPlay(item)">
        <i class="fa fa-music" aria-hidden="true"></i>
        <span class="songName">{{item.songname}}-{{item.singer[0].name}}</span>
      </li>
...
export default {
  ...
  methods: {
    ...
    getSearchResult () {
      if (this.flag) return
      let url1 = encodeURI(`http://${this.$store.state.play.path}:4000/item/search?keyword=${this.keyword}&page=${this.page}`)
    ...
    compare (keyword) {
      let history = JSON.parse(localStorage.getItem('listen')).map((item) => {
        if (item.musicData) {
          return { name: item.musicData.singer[0].name, mid: item.musicData.singer[0].mid }
        } else {
          return { name: item.singer[0].name, mid: item.singer[0].mid }
        }
      })
    ...
    goPlay (songInfo) {
      let a = this.playList.some((item) => {
        return item.songmid === songInfo.songmid
      })
      if (a) return
